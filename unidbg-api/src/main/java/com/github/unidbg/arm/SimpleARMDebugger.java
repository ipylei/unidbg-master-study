    ...
    protected final void loop(Emulator<?> emulator, long address, int size, DebugRunnable<?> runnable) throws Exception {
        ...
        try {
            if (address != -1) {
                ...
                //打印断点断住位置
                //System.out.print(String.format("onBreak >>> Tracing instruction at 0x%x, instruction size = 0x%x\n", address, size));
        ...
        while ((line = scanner.nextLine()) != null) {
            ...
            try {
                ...
                if (line.startsWith("m")) {
                    ...
                    //读取并打印寄存器中的值：如:mr0、mr1...
                    //注意这里是把寄存器中的值作为指针! 打印该指针指向的内容。

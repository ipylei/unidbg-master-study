...
public class UnidbgPointer extends Pointer implements PointerArg {
    ...
    public final long peer;           //指针指向的内容，该内容的首地址
    private final int pointerSize;    //指针大小，区分是32位，还是64位；还有个私有属性size是指向的数据块大小
    ...
    //读取指定位置(offset)、指定大小(length)的数据到参数(buf)中指定处(index)
    @Override
    ...
    //读取指定偏移offset处的数据，读取大小arraySize
    @Override
    ...
    //把某指针的地址写入到该指针指向的内容处。如：模仿linker链接so文件时，用此方法可以很方便
    @Override

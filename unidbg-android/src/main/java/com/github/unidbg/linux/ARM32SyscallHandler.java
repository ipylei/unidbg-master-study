...
public class ARM32SyscallHandler extends AndroidSyscallHandler {
    ...
    private int bionic_clone(Emulator<?> emulator) {
        ...
        Pointer child_stack = context.getR1Pointer(); //子线程栈
        Pointer pid = context.getR2Pointer();
        Pointer tls = context.getR3Pointer();
        Pointer ctid = context.getR4Pointer(); //线程id
        UnidbgPointer fn = context.getR5Pointer(); //线程要执行的入口(即线程处理函数)
        UnidbgPointer arg = context.getR6Pointer(); //给线程的一些参数
        List<String> list = new ArrayList<>();
        if ((flags & CLONE_VM) != 0) {
            list.add("CLONE_VM");
        }

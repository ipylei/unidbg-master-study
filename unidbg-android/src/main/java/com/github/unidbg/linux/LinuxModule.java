...
public class LinuxModule extends Module {
    ...
    LinuxModule(long virtualBase, long base, long size, String name, SymbolLocator dynsym,
        ...
        this.virtualBase = virtualBase;               //内存中的虚拟地址
        this.dynsym = dynsym;                         //该模块对应的符号表
        this.unresolvedSymbol = unresolvedSymbol;     //还未重定位的符号
        this.initFunctionList = initFunctionList;     //存放初始化函数
        this.armExIdx = armExIdx;
        this.ehFrameHeader = ehFrameHeader;
        this.symbolTableSection = symbolTableSection;
        this.elfFile = elfFile;                       //对应的elfFile
        this.dynamicStructure = dynamicStructure;     //对应的动态段
    }
    ...
    void callInitFunction(Emulator<?> emulator, boolean mustCallInit) throws IOException {
        ...
        int index = 0;
        // 否则，就要挨个执行初始化函数了，这个initFunctionList就是我们上篇分析过的初始化函数列表

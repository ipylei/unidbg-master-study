...
public class MainActivity_LESSON extends AbstractJni {
    private final AndroidEmulator emulator;
    private final VM vm;
    private final Memory memory;
    private final Module module;

    public MainActivity_LESSON() {
        ...
        DalvikModule dalvikModule = vm.loadLibrary(new File("unidbg-android/src/test/java/com/dta/lesson32_36/lesson32/libdogpro.so"), false);
    ...
    public static void main(String[] args) {
        ...
        MainActivity_LESSON mainActivity = new MainActivity_LESSON();
    ...
    public int callIntMethodV(BaseVM vm, DvmObject<?> dvmObject, String signature, VaList vaList) {
        ...
        if(signature.equals("java/util/zip/ZipFile$ZipFileInputStream->read([B)I")){
            InputStream inputStream = (InputStream) dvmObject.getValue();
            byte[] bytes = (byte[]) vaList.getObjectArg(0).getValue();
            try {
                int read = inputStream.read(bytes);

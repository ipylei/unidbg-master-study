...
function monitor_constants(targetSo) {
    let const_array = [];
    let const_name = [];
    let const_addr = [['MD5 K table', '0x17720']];
...
function hook_suspected_function(targetSo) {
    const funcs = [['sub_B7D4', '函数sub_B7D4疑似哈希函数，包含初始化魔数的代码。', '0xb7d5'], ['sub_BDB4', '函数sub_BDB4疑似哈希函数，包含初始化魔数的代码。', '0xbdb5'], ['sub_D5CC', '函数sub_D5CC疑似哈希函数，包含初始化魔数的代码。', '0xd5cd'], ['sub_DA34', '函数sub_DA34疑似哈希函数，包含初始化魔数的代码。', '0xda35']];
    for (var i in funcs) {
...
function main() {
    var targetSo = Module.findBaseAddress('libjni.so');
